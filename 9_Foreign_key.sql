...
create table addresses(
    ...
    state varchar(100),
    pincode varchar(10),
    constraint fk_user foreign key (user_id) references user4(id) on delete cascade
    -- on cascade means: if any id row is deleted from parent table then the address will also deleted
    -- you can also user "set null cascade" if row deleted from parent then now it will set as null
    );
...
-- if we delete the addreess then user will not delete

delete from user4 where id=3;
-- if we delete user then the address will also delete
